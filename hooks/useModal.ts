 ...
 * @property {(content: React.ReactNode, position: ModalPosition, options?: {replace?: boolean}) => void} openModal - 새 모달을 열거나 최상위 모달을 교체
...
export function useModal() {
   ...
   * @param options 추가 옵션 ({ replace: boolean })
   */
  const openModal = (
    content: React.ReactNode,
    ...
    const newModal: InternalModal = { content, position, id: generateId() };
